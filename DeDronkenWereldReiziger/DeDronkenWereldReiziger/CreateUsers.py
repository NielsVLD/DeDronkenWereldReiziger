...
def mousePressed():
    ...
    if(functions.onRect(width/2 - 100, 725, 200, 50) and gameData['errorMessage2'] == False):
        for i in gameData['users']:
            if(i != '' and i != None):
                players += 1
                
        if(players == int(jsonData[0]['amountOfUsers'])):
            shown = False
        else:
            players = 0
...
def keyPressed():
    ...
    if(key == ENTER ):   
        if(users == 0):
            ...
            if(int(jsonData[0]['amountOfUsers']) > int(users)):
